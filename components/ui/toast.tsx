  ...
  return (
    <div className="fixed bottom-0 right-0 z-50 p-4 space-y-4 w-full max-w-sm">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`p-4 rounded-md shadow-lg transition-all transform translate-y-0 opacity-100 flex justify-between items-start
        ...
        >
          ...
          <button onClick={() => dismiss(toast.id)} className="ml-4 opacity-70 hover:opacity-100">
            <X size={18} />
          </button>
        </div>
      ))}
